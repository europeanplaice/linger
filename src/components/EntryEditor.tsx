...
function DriveBadgeIcon() {
  return (
    <svg aria-hidden="true" width="22" height="20" viewBox="0 0 87.3 78" style={{ flexShrink: 0 }}>
      ...
      <circle cx="74" cy="66" r="14" fill="#34A853"/>
      <path d="M68 66l4 4 7-7" fill="none" stroke="#fff" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
    </svg>
  )
}

function S3BadgeIcon({ status }: { status?: 'pending' | 'synced' | 'failed' | 'disabled' }) {
  return (
    <svg aria-hidden="true" width="22" height="20" viewBox="14.7 23 52 36" style={{ flexShrink: 0 }}>
      ...
      {status === 'failed' ? (
        <>
          <circle cx="58" cy="52" r="11" fill="#D13212"/>
          <path d="M54 48l8 8m0-8l-8 8" stroke="#fff" strokeWidth="2.5" strokeLinecap="round"/>
        </>
      ) : status === 'synced' ? (
        <>
          <circle cx="58" cy="52" r="11" fill="#34A853"/>
          <path d="M53 52l3 3 6-6" fill="none" stroke="#fff" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
        </>
      ) : (
        <circle cx="58" cy="52" r="8" fill="#F7981F" className="s3-sync-pending" />
      )}
    </svg>
  )
}
