...
import { mirrorEntrySave, mirrorEntryDelete } from '../../../_shared/s3Settings'
  ...
  try {
    ...
    if (body.fileId && (body.force || body.baseVersion !== null)) {
      ...
      try {
        ...
        context.waitUntil(mirrorEntrySave(accessToken, sessionId, session, context.env, date, body.content, savedMeta.id, savedMeta.version))
...
export const onRequestDelete: PagesFunction<Env, 'date', Data> = async (context) => {
  ...
  try {
    ...
    context.waitUntil(mirrorEntryDelete(accessToken, sessionId, session, context.env, date))
